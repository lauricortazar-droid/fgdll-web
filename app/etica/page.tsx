...
function downloadReceipt(receipt: EthicsReceipt) {
  ...
  context.font = "800 55px ui-monospace, monospace";
  context.fillText(receipt.trackingKey.replace(/(.{4})/g, "$1 ").trim(), 90, 525);
...
export default function EthicsPage() {
  ...
  return <>
    ...
    <main className="subpage ethics-page ethics-spa-page">
      ...
      <nav className="ethics-spa-index" aria-label="Contenido del portal"><div className="shell"><a href="#identidad"><b>01</b>Identidad</a><a href="#categorias"><b>02</b>Conductas</a><a href="#salvaguardas"><b>03</b>Salvaguardas</a><a href="#disciplina"><b>04</b>Disciplina</a><a href="#incidencias"><b>05</b>Incidencias</a><a href="#transparencia-proactiva"><b>06</b>Transparencia</a><a href="#reporte"><b>07</b>Reporte</a></div></nav>
      ...
      <section className="section ethics-tracking" id="seguimiento"><div className="shell report-layout"><div><span className="eyebrow">Seguimiento privado</span><h2>Consulta el estado de tu reporte.</h2><p>Introduce exactamente el folio de 16 dígitos y la contraseña de 12 dígitos que recibiste. La consulta no muestra notas internas ni datos administrativos.</p></div><form className="report-form" onSubmit={track}><label>Folio<input name="publicFolio" required inputMode="numeric" placeholder="0000 0000 0000 0000" /></label><label>Contraseña privada<input name="trackingKey" required inputMode="numeric" autoComplete="off" placeholder="0000 0000 0000" /></label><button className="button button-gold" disabled={trackingLoading}>{trackingLoading ? "Consultando…" : "Consultar seguimiento"}</button>{trackingError && <p className="form-error" role="alert">{trackingError}</p>}{tracking && <div className="tracking-result"><strong>Estado: {tracking.report.status}</strong><small>Folio {String(tracking.report.public_folio).replace(/(.{4})/g, "$1 ").trim()}</small>{tracking.events.map((item, index) => <article key={`${item.created_at}-${index}`}><b>{item.public_message}</b><time>{item.created_at}</time></article>)}</div>}</form></div></section>

      <section className="ethics-closing"><div className="shell"><span className="eyebrow light">Compromiso institucional</span><h2>La luz también se cuida con límites.</h2><p>Conocer las reglas, documentar con honestidad y actuar sin represalias fortalece el servicio y protege a quien todavía está aprendiendo a pedir ayuda.</p><div><Link className="button button-gold" href="/lider">Volver al Portal del Líder →</Link><a className="button button-ghost" href="tel:911">Emergencia: llamar al 911</a></div></div></section>
    </main>
    <SubFooter />
  </>;
}
